...
### 1) 掲載データと表示対象
- 元データ件数は `1,278` 件（`src/data/booths.json`）。
- アプリ表示時は規約都合で `EXPRESS WORK-*` を除外し、表示対象は `1,261` 件。
- 表示ブランド内訳（除外後）: `STATION BOOTH 710 / テレキューブ 261 / CHATBOX 209 / CocoDesk 81`。
- 最寄駅データは補完済みで、現行データ上の `station` 空欄は `0` 件。

### 2) データ取得の実挙動
- 起動時はバンドル済みローカルJSONで即時描画し、その後 Supabase の `booths` テーブルを全件ページング取得して差し替え。
- Supabase が未設定/失敗時はローカルJSONへ自動フォールバック。
- ヘッダーにデータソースが表示され、ローカル時は `（ローカル）` が付与される。

### 3) 検索・絞り込み仕様
- 検索対象: `施設名 / 最寄駅 / 住所 / 都道府県 / 詳細 / ブランド / 会社名`。
- 日本語・英語入力を想定し、`NFKC正規化 + 小文字化 + 空白/記号吸収` で照合。
- `phone / booth / telecube / cocodesk / chatbox` などの別名展開、URLスラッグ・英字名称も検索対象に含む。
- ブランドフィルタと検索は排他（ブランド操作で検索結果をクリア）。

### 4) 地図・リスト・ルート表示
- 地図: 画面内のブースを表示（通常時最大300件、検索時も最大300件）。
- リスト: 通常は現在地アンカーから近い順100件、検索時は関連度順200件。
- ルート:
  - Web: Valhalla 歩行者ルートを描画（失敗時は直線目安表示）。
  - iOS/Android: 外部 Google マップの徒歩ナビを起動。

### 5) 課金と機能制限
- 無料プラン: 地図/リスト表示、検索、ブース詳細閲覧。
- プレミアム（1ヶ月 `¥200` / 12ヶ月 `¥1,200`）で以下を開放:
  - 徒歩ルート案内
  - 予約ページリンク
- ペイウォール内で購入復元・クーポンコード適用に対応。
...
> データ取得はハイブリッド方式: 起動時に Supabase の `booths` テーブルを全件ページング取得し、失敗/未設定時はバンドル済み JSON にフォールバック。
